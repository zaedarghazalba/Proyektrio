...
export function Services() {
  ...
  return (
    <section id="services" className="py-32 px-8 md:px-16 bg-black">
...
function ServiceCard({
  ...
  return (
    <motion.div
      variants={variants}
      className="group relative p-8 md:p-12 bg-g1 border border-g3 hover:border-g5 transition-colors duration-500 overflow-hidden"
      data-cursor="hover"
    >
      ...
      {/* Index Number */}
      <motion.span
        className="block font-mono text-label-xs text-g4 mb-8"
        initial={{ opacity: 1 }}
      ...
      <motion.span
        className="absolute top-12 left-12 font-mono text-label-xs text-white opacity-0"
      ...
      {/* Icon */}
      <div className="text-g5 group-hover:text-white transition-colors duration-500 mb-8">
        {service.icon}
      </div>

      {/* Title */}
      <h3 className="font-display text-display-sm text-white uppercase mb-4">
        {t(`${service.key}_title`)}
      </h3>

      {/* Description */}
      <p className="font-body text-body-base text-g5 leading-relaxed">
        {t(`${service.key}_desc`)}
      </p>

      {/* Arrow */}
      <motion.div
        className="absolute bottom-12 right-12 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
        initial={{ x: -10 }}
        whileHover={{ x: 0 }}
      >
